...
program process
    ...
    real(kind=dp), allocatable :: times(:), dry_diameters(:), num_concs(:), &
         ...
         crit_rhs(:), scs(:), num_dist(:), diam_bc_dist(:,:), &
         diam_sc_dist(:,:), scs_varying_sigma(:), &
         crit_rhs_varying_sigma(:), diam_sc_varying_sigma_dist(:,:)
  ...
  !for critical RH supersaturation with varying surface tension
          ...
          diam_sc_varying_sigma_dist = bin_grid_histogram_2d(diam_grid, dry_diameters, &
               sc_grid, scs_varying_sigma, num_concs)
